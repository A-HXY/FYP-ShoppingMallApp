<template>
    <div class="Box">
      ...
      <div class="funBtn">
        <div>
          <el-button type="success" @click="AddCategoryBtn" style="margin-left: 10px;margin-right: 10px;">Add Category</el-button>
        </div>
      </div>
      
      <!-- 展示用户信息的表格 -->
      <el-table :data="tableDataFormat" class="tableBox">
        <el-table-column label="Category ID" prop="gcategory_id" align="center"/>
        <el-table-column label="Category Name" prop="gcategory_name" align="center"/>
        <el-table-column label="Status" prop="status" align="center"/>
        <el-table-column label="Options" fixed="right" align="center">
          <template #default="scope">
            <el-button size="mini" type="success" @click="editBtn(scope.row)" icon="el-icon-edit">Edit</el-button>
            <el-button size="mini" :type="scope.row.status == 1 ? 'danger':'success'" @click="changeStatusBtn(scope.row)" icon="el-icon-edit">{{scope.row.status == 1 ? 'disable':'pass'}}</el-button>
          </template>
        </el-table-column>
      </el-table>
